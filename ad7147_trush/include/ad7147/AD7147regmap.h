#ifndef __H_AD7147R__
#define __H_AD7147R__
#include <stdint.h>
//========================================================================================
//File AD7142RegMap.h
...
#pragma pack(push,1)
enum class cin_conn_setup: unsigned int{
  not_connected=0,negative,positive,bias
};
template<typename Bitfield> struct bitfield{
  bitfield(){
    memset(this,0,sizeof(Bitfield));
  };
};
struct cin0to6_connection:bitfield<cin0to6_connection>{
  cin_conn_setup cin3:2,cin2:2,cin1:2,cin0:2;
  cin_conn_setup unused:2,cin6:2,cin5:2,cin4:2;
};
struct cin7to12_connection:bitfield<cin7to12_connection>{
  cin_conn_setup cin10:2,cin9:2,cin8:2,cin7:2;
  cin_conn_setup cin12:2,cin11:2;
  unsigned char se_connection_setup:2,neg_afe_offset_disable:1,pos_afe_offset_disable:1;
};
struct afe_offset_control:bitfield<afe_offset_control>{
  uint8_t neg_afe_offset_swap:1,unused:1,neg_afe_offset:6;
  uint8_t pos_afe_offset_swap:1,unused2:1,pos_afe_offset:6;
};
struct sensitivity_control:bitfield<sensitivity_control>{
  uint8_t   unused:1,neg_peak_detect:3,neg_threshold_sensitivity:4;
  uint8_t unused2:1, pos_peak_detect:3,pos_threshold_sensitivity:4;
};
struct stage_configuration_registers:bitfield<stage_configuration_registers>{
  cin0to6_connection stage_connection_0to6;
  cin7to12_connection stage_connection_7to12;
  afe_offset_control afe_offset;
  sensitivity_control sensitivity;
  uint32_t offset_low, offset_high;
};
struct spi_command_word{
  uint8_t reg_hi:2,rw_flag:1,x4:1,x3:1,x2:1,x1:1,x0:1;
  uint8_t reg_lo;
